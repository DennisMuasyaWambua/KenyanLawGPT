...
# file facts, clause-library language, and retrieved authority.
...
3. THAT what is deponed to herein is true to the best of my knowledge save
   for files based on information the sources whereof have been disclosed.
...
class DraftingEngine:
    ...
    async def _file_facts(self, tenant_id: str, file_id: str) -> str:
        try:
            async with dbx.tenant_tx(self.pool, tenant_id) as conn:
                row = await conn.fetchrow(
                    """SELECT m.reference, m.title, m.description, m.practice_area, m.court,
                       ...
                       FROM files m LEFT JOIN clients c ON c.id = m.client_id
                       WHERE m.id = $1""", file_id)
            if not row:
                return ""
            return (f"File reference: {row['reference']}\nFile: {row['title']}\n"
            ...
            log().warning("file facts lookup failed: %s", exc)
            return ""
    ...
    async def draft(
        ...
        facts = await self._file_facts(tenant_id, file_id) if file_id else ""
        ...
        try:
            chunks, _ = await self.retriever.retrieve(tenant_id, query, top_k=6, file_id=file_id)
            citations = chunks
            law_context = "\n\n".join(
        ...
        system = (
            "You are C. Karwitha C.K Advocates' drafting engine for Kenyan legal documents. Produce a "
            ...
            "and file facts support, and leave square-bracket placeholders for anything "
